module.exports.run = async (client, message, args) => {
    ...
    if (!user) {
        client.send.missing(message, `You have not included a user to remove a role from`, `12`);
        return client.send.log(message);
    }
    ...
    if (!member.roles.cache.has(role.id)) {
        client.send.missing(message, `User doesn't have that role`, `15`);
        return client.send.log(message);
    }
    ...
    try {
        ...
        const embed = client.send.embed()
            .setTitle(`Role removal successful`)
...
module.exports.code = {
    ...
    group: "moderation",
    usage: ["/PREFIX/removerole [USER] [ROLE or ROLE NAME]"],
    accessableby: "Villagers with MANAGE ROLES OR PERMISSIONS permission",
    aliases: ["removerole", "remr", "remrole"]
}