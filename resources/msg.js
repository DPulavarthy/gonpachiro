...
module.exports = async (client, message, prefix, guild) => { // Message event
    ...
    async function check(command) {
        ...
        try { command.run(client, message, args, prefix); } catch (error) { client.error(error); return message.channel.send(client.src.comment(`There was an error trying to execute that command!\nERROR: ${error}`)); };
        //client.statcord.postCommand(postcommand, message.author.id);
